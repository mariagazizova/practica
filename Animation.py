...
class AnimateFieldDisplay:
    ...
    def activate(self):
        ...
        self._fig, self._ax = pylab.subplots(figsize=(10, 6.5))
    ...
    def drawProbes(self, probesPos: List[int]):
        ...
        self._ax.plot(
            np.array(probesPos) * self._dx, [0] * len(probesPos), self._probeStyle
        )
    ...
    def drawSources(self, sourcesPos: List[int]):
        ...
        # Отобразить положение пробника
        self._ax.plot(
            np.array(sourcesPos) * self._dx, [0] * len(sourcesPos), self._sourceStyle
        )
    ...
    def drawBoundary(self, position: int):
        ...
        """
        self._ax.plot(
            [position * self._dx, position * self._dx],
...
class Source(Source1D):
    ...
    def getH(self, time):
        return -(self.Sc / (self.W0 * self.mu)) * (
            self.source.getE(self.sourcePos, time)
            - self.source.getE(self.sourcePos - 1, time)
        )

    def getE(self, time):
        return (self.Sc / np.sqrt(self.eps * self.mu)) * (
            self.source.getE(self.sourcePos - 0.5, time + 0.5)
            + self.source.getE(self.sourcePos + 0.5, time + 0.5)
        )
