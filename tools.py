...
def sampleLayer(layer_cont: LayerContinuous, sampler: Sampler) -> LayerDiscrete:
    ...
    end_discrete = (
        sampler.sample(layer_cont.xmax) if layer_cont.xmax is not None else None
    )
    return LayerDiscrete(
        start_discrete, end_discrete, layer_cont.eps, layer_cont.mu, layer_cont.sigma
    )
